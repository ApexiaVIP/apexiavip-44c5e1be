-- Which store app, if any, a member has signed in from. Stamped by sms-2fa
-- (service role) when a code is verified inside the iOS or Android app; never
-- cleared by a later web sign-in, so "no app" means they have never used one.
-- app_nudged_at records the last time a text invited them to install it.
